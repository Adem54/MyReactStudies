...
a=24.55;
let b:any;//ARtik be degskenine istedigmi her type e cevirebiliriz program icerisinde 
...
const logMyUser=(firstName? :string, lastName:string="Erbas")=>{
    console.log(firstName+ "  "+ lastName);
}
...
const myFunc=(number1:45 | 55, number2:65 | 12)=>{
    console.log(number1+number2);
}
myFunc(45,12);
//Yine fonksiyonlarda custom type larimizi da kullanabiliriz...
type Color={
    name:string;
    hex:string;
}
const myNewFunc=(color:Color)=>{
        console.log(color);
}
myNewFunc({
    name:"black",
...
//OOP OZELLIKLERI
//INTERFACE I DE BIR CUSTOM TYPE GIBI KULLANMAK...
interface Employee {
    empCode?:number;//empCode optioanal yani verilmese de olur demek
...
//Bu sekilde interface i de bir tip olarak kullanabiliriz...
//Burda da yine optional veya bazi degiksenleri readonly olusturulmasini istersek de ayarlayabilirz.
//  empCode?:number;//empCode optioanal yani verilmese de olur demek
//VE biz name field-property degiskenini readonly yani disardan veri girilemesin(sadece constructor araciligi ile)
//  readonly  name:string;
//Artik disardan deger atanamaz
//newEmployee.name="Kamil";//hata aliriz...//Cannot assign to 'name' because it is a read-only property.

//EXTENDIGN-INTERFACE LERIMIZI GENISLETMEK
interface Person {
    name:string;
    age:number;
}
...
//Javscript karsiliginda interface vs,extends vs bunnlar yok sadece typescript kodu icinde var
//Watch moduna alarak anlik olarak, yazdiklarimizi typescripti dinliyor ve bizim her yazdgimz
//degisikler otomatik derlenecek...
//Biz otomaik yazdimiz her tscript kodunun jscripte cevrilmesi icin -watch moduna aliriz
//tsc main.ts -w deriz...

//Interface leri implemente etmek
interface IStudent {
    studentCode:number;
    name:string;
    age:number;
...
class Student implements IStudent{
    ...
    name: string;
    age: number;
    getMoney= (num1: number) => {
        return 10000;
    };

    constructor(studentCode:number,name:string,age:number){
...
let newStudent=new Student(1,"Zehra",9); //Seklinde kullaniriz..
console.log(newStudent.getMoney(2));//10000
